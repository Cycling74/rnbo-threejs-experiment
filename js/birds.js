...
export function createBirdFlock(position, flockSize = 3) {
...
// Animate birds (call this in your animate loop)
export function animateBirds(scene, birdFlocks, deltaTime) {
    birdFlocks.forEach((flock, flockIndex) => {
        // Age the flock
        ...
        if (flock.userData.age > flock.userData.maxAge) {
            scene.remove(flock);
            birdFlocks.splice(flockIndex, 1);
            return;
        }
...
export function spawnBirdsAtPosition(scene, birdFlocks, worldPosition) {
    ...
    const flock = createBirdFlock(spawnPosition);
    scene.add(flock);
